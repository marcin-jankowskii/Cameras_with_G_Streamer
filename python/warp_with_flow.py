...
def parse_args():
    ...
    ap.add_argument("--fillmode", default="smart", choices=["none","base","smart"],
                    help="How to fill black borders after warping: none | base (use original A2B) | smart (use warped A2B to fill invalid-from-A regions)")
    ap.add_argument("--feather", type=int, default=0,
                    help="Feather width (pixels) for blending warped vs base when fillmode=base/smart")
    # direct-from-A mode
        ...
        if args.mode == "a2b":
            ...
            if args.autosign:
                ...
                if args.apply_if_better and not (chosen_err <= base_err - args.better_delta):
                    ...
                    if args.fillmode == "base":
                        warped = blend_with_base(warped_raw, imgA2B, valid, feather=args.feather)
                    elif args.fillmode == "smart":
                        # wypełnij niepoprawne próbki z A pikselami z warp(A_to_B) i zmieszaj wg złączonej maski
                        warped_flow, valid_flow, _, _ = warp_a2b_to_b(imgA2B, flow_r, sign=chosen_sign, borderMode=borderMode, borderValue=(args.fill,args.fill,args.fill))
                        combined = warped_raw.copy()
                        holes = (valid == 0) & (valid_flow == 1)
                        if np.any(holes):
                            combined[holes] = warped_flow[holes]
                        valid_combined = (valid | valid_flow).astype(np.uint8)
                        warped = blend_with_base(combined, imgA2B, valid_combined, feather=args.feather)
                    else:
                        warped = warped_raw
                ...
                if args.fillmode == "base":
                    ...
                    warped_flow, valid_flow, _, _ = warp_a2b_to_b(imgA2B, flow_r, sign=args.sign, borderMode=borderMode, borderValue=(args.fill,args.fill,args.fill))
                    combined = warped_raw.copy()
                    holes = (valid == 0) & (valid_flow == 1)
                    if np.any(holes):
                        combined[holes] = warped_flow[holes]
